...
def convert_planet_list(active_campaigns: list[CampaignPlanet]) -> str:
    ...
    for campaign in active_campaigns:
        direction = "under attack by" if campaign.defense else "being defended against an assault by"
        ...
        current_planets += f"The planet's biome is a {campaign.biome.slug}. Described as: {campaign.biome.description}" if campaign.biome else ""
    ...
    for major_order in active_major_orders:
        ...
        for task in major_order.tasks:
            if task.target_value > 0 and task.current_value != task.target_value:
                ...
                current_major_orders += f"We have completed the major order.\n"
...
def convert_current_event_list(current_events: list[CurrentEvent]) -> str:
    ...
    for event in current_events:
        if event.message and event.title:
            current_events += f"{event.title}: {event.message}\n"
