...
from tools.helldivers.training_manual_types import CampaignPlanet, CurrentStatus, MajorOrder, MajorOrderTask, News


def get_campaigns() -> list[CampaignPlanet]:
    """Get a list of all active planets that are currently being contested"""
    campaigns_raw = requests.get("https://helldiverstrainingmanual.com/api/v1/war/campaign")
...
def get_major_orders() -> list[MajorOrder]:
    ...
    major_orders_raw = requests.get("https://helldiverstrainingmanual.com/api/v1/war/major-orders")
    ...
    major_orders = [
        MajorOrder(
            ...
            tasks=[
                MajorOrderTask(type=task["type"], target_value=task["values"][2], current_value=progress) for task, progress in 
                zip(major_order["setting"]["tasks"], major_order["progress"])
            ]
...
def get_current_status() -> CurrentStatus:
    ...
    current_status_raw = requests.get("https://helldiverstrainingmanual.com/api/v1/war/status")
...
def get_past_week_news(state: State) -> list[News]:
    ...
    news_dict = {}
    for i in range(7):
        daily_news_raw = requests.get(f"https://helldiverstrainingmanual.com/api/v1/war/news?from={state['current_status'].time - i * 86400}").json()
